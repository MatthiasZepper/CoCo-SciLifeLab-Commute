...
config_path_geotiffs <- "./data/nh_01_Stockholms_lan_WGS84_geotiff"
```
...
```{r}
coordinates <-
  scan(textConnection(opq ("Stockholm")$bbox), sep = ",")
  lat <- diff(coordinates[c(1, 3)]) * 111.412
  long <- diff(coordinates[c(2, 4)]) * 55.8
  area <- lat * long
print(paste0(
  "ΔLatitude: ",
  round(lat,1),
...
This however refers only to the inner city of Stockholm and in particular omits many areas in the west like Järfalla, Upplands Väsby and such. Therefore we will download the data for the whole Stockholms län. *SE-AB* is the ISO3166-2 standardized code for the province, so it is actually safer than just entering *Stockholms län*, but in this case both work fine: Using the same function as above to verify: 

```{r}
coordinates <-
  scan(textConnection(opq ("Stockholms län")$bbox), sep = ",")
  lat <- diff(coordinates[c(1, 3)]) * 111.412
  long <- diff(coordinates[c(2, 4)]) * 55.8
  area <- lat * long
print(paste0(
...
```
This now increases the area by 50, but since the more densely populated areas constitute more data, it only increases the amount of downloaded data by approximately 3.5 fold. 
...
if (!file.exists ("data/Silicate_sthlm_map_cycle.Rds") |
  ...
  sthlm_map_cycle <- osmdata::opq("Stockholms län", timeout=600) %>%
    osmdata::add_osm_features(
      features = c(
        "\"amenity\"=\"bicycle_parking\"",
        "\"cycleway\"=\"lane\"",
...
Therefore, if we wish to consider elevation data for routing, we need to obtain it from another source and integrate it with our cycle map. The `osmdata` package can integrate elevation data in GeoTIFF format, an extended image file format that represents elevation as grayscale values, using the convienient function `osmdata::osm_elevation`. 

It is optimized for data from the [CGIAR-CSI SRTM dataset](https://srtm.csi.cgiar.org/srtmdata), a post-processed derivation of the finished-grade 3 arc-second SRTM data released by the *National Aeronautics and Space Administration* (NASA) and distributed by the *United States Geological Survey* (USGS). Since it is based on satellite data, it lacks information for areas that are rarely overflown by satellites. In northern Europe, the boundary of this dataset is at about 60° latitude. Stockholm is therefore still covered (albeit at a low resolution), but not northern Sweden. Its resolution is also pretty low for routing within cities, all bridges are for example considered to be almost on sea level.

```{r eval=FALSE}
#GeoTIFF downloaded from https://srtm.csi.cgiar.org/wp-content/uploads/files/srtm_5x5/TIFF/srtm_40_01.zip
sthlm_map_cycle_scevl <- osmdata::osm_elevation(sthlm_map_cycle_sc, elev_file = "./data/srtm_40_01/srtm_40_01.tif")
}
```
...
However, if you try to simply add the `nh_01_Stockholms_lan_Sweref_99_TM_geotiff` (or any other dataset from Lantmäteriet) you will unfortunately fail: *In showSRID(SRS_string, format = "PROJ", multiline = "NO", prefer_proj = prefer_proj) : Discarded datum SWEREF99 in Proj4 definition*

The reason is, that our earth is of course not flat and in order to coerce geographical information into a two-dimensional representation like a flat image file, a projection needs to be used. Most [Swedish datasets make use of the SWEREF99 coordinate system](https://www.lantmateriet.se/en/maps-and-geographic-information/gps-geodesi-och-swepos/Referenssystem/Tredimensionella-system/SWEREF-99/). This is Swedens national coordinate system and is only used in and around Sweden. The reason to maintain an own system is that most projections are optimized for smaller latitudes and an increasingly strong distortion is accepted towards the poles. For a country like Sweden that extents beyond the polar circle, a more favorable projection is hence advisable, but makes it difficult to integrate with most other data sources.    

In order to use these coordinates together with software that works with the WGS84 coordinate system, an internationally widely used standard that also OSM adheres to, [conversion using a Gauss-Kreuger projection](https://pap.as/sweref/) is needed.

Some software can do the transformation on the fly, if the appropriate sidecar file with spatial information is provided (the accompanying .tfw files in the folder), but `osm_elevation` depends solely on the GeoTIFF header, so a prior conversion of the TIFF files e.g. with [`gdalwarp`](https://gdal.org/programs/gdalwarp.html) is necessary. Integration into R is offered by the [`gdalio`](https://github.com/hypertidy/gdalio) package, but I just used a small command line script to convert the TIFF files. 
...
Unfortunately, the `osmdata::osm_elevation` function will also not work with multiple files, so I wrote this custom derivative function to annotate my data with elevation data:

```{r eval=FALSE}
if(config_enrich_elevation) {
  
enrich_elevation <- function(dat,geotiff_path, pattern=NULL, replace_na=FALSE){

  files <- as.list(list.files(config_path_geotiffs,pattern = pattern, full.names = TRUE))
  z_values <- lapply(files,function(x,dat){osmdata::osm_elevation(dat,elev_file=x)$vertex$z_},dat=dat)
  z_values <- do.call("cbind",z_values)
  z_values <-  rowMeans(z_values,na.rm = TRUE)

  if(replace_na){
    z_values[is.na(z_values)] <- median(z_values,na.rm = TRUE)
  }
  
  dat$vertex$z_ <- as.double(z_values)
  dat$vertex <- dat$vertex[, c("x_", "y_", "z_", "vertex_")]
  return(dat)
}

  sthlm_map_cycle_scevl <- enrich_elevation(sthlm_map_cycle_sc,config_path_geotiffs,pattern="[0-9]_WGS84.tif",replace_na = TRUE)
...
```{r}
library("dodgr")
sthlm_graph_cycle_scevl_noturn <- dodgr::weight_streetnet (sthlm_map_cycle_scevl, wt_profile = "bicycle", turn_penalty = FALSE)
...
Weighting for time-based routing introduces new "compound" edges and requires re-labelling junction vertices, through appending either "\_start" or "\_end". Recommended practice for routing in such cases is to select routing vertices (origins and destinations) from a standard weightedgraph (that is, one generated with `turn_penalty = FALSE`), and then to modify these routing vertices.

The time-weighted graph has additional compound edges used to reflect the penalty for turning across traffic. Let's now presume we want to calculate distances between some number of randomly-selected street junctions. The junctions may readily be extracted through the [`dodgr_contract_graph()` function](https://atfutures.github.io/dodgr/reference/dodgr_contract_graph.html), which reduces the graph to junction vertices only. The junction vertices of `sthlm_graph_cycle_scevl` are re-labelled as described above to separate incoming from outgoing edges (through appending `_start` and `_end` to vertex names), and so may not be used for routing. Instead, routing points should be taken from the contracted version of the original graph.
...
```{r}
dim(sthlm_graph_cycle_scevl_noturn)
...
Contract the graph to reduce complexity of the network:

```{r}
dim(sthlm_graph_cycle_scevl)
sthlm_graph_cycle_scevl_noturn <- dodgr::dodgr_contract_graph(sthlm_graph_cycle_scevl_noturn)
sthlm_graph_cycle_scevl <- dodgr::dodgr_contract_graph(sthlm_graph_cycle_scevl)
dim(sthlm_graph_cycle_scevl)
saveRDS(sthlm_graph_cycle_scevl, file = "data/Dodgr_sthlm_graph_cycle.Rds")
saveRDS(sthlm_graph_cycle_scevl_noturn, file = "data/Dodgr_sthlm_graph_cycle_noturn.Rds")
``` 